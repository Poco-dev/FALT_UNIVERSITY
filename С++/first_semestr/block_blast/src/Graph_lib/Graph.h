...
struct Color
{
  ...
  // НОВЫЙ: конструктор для RGB цветов
...
// Вспомогательные функции для цветов (после объявления Color!)
inline Color rgb_color(int r, int g, int b) {
    return Color(r, g, b);
}

// Популярные цвета как inline функции
inline Color pink() { return rgb_color(255, 105, 180); }
inline Color orange() { return rgb_color(255, 165, 0); }
inline Color purple() { return rgb_color(128, 0, 128); }
inline Color brown() { return rgb_color(139, 69, 19); }
inline Color gray() { return rgb_color(128, 128, 128); }
inline Color light_blue() { return rgb_color(173, 216, 230); }
inline Color light_green() { return rgb_color(144, 238, 144); }

// Утилиты для рисования (объявления)
namespace Draw {
    ...
    void polygon(std::vector<Point> points, Color c);
    void text(int x, int y, const std::string& str, Color c, int font_size = 14);
